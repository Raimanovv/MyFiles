...
Пример:
Введите шаблон поздравления, в шаблоне можно использовать конструкцию
{name} и {age}: С днём рождения, {name}! С {age}-летием тебя!
...
while True:
    text = input('Введите поздравление используя {age} и {name}:')
    if '{age}' and '{name}' in text:
        break
    else:
        print('Нету {age} или {name}')

names = input('Список людей через запятую:').split(', ')
age = input('Возрост через пробел:').split(' ')

for i in range(len(names)):
    print(text.format(age=age[i], name=names[i]))

ff = [' '.join([names[i], age[i]]) for i in range(len(names))]
